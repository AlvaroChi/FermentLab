...
with st.expander("Parametri di elaborazione"):
    ...
    smoothing_minutes = col_a.slider("Filtro volume (min)", 1, 30, 5)
    baseline_offset_minutes = col_b.slider(
        "Ignora i primi minuti", 0, 180, 0, 5
    )
    col_c, col_d = st.columns(2)
    baseline_minutes = col_c.slider("Durata baseline (min)", 1, 30, 5)
    rate_window_minutes = col_d.slider("Finestra velocità (min)", 5, 120, 30, 5)
...
temperature_figure.update_yaxes(title_text="Velocità (%/h)", secondary_y=True)
