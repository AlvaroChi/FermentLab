...
<script>
  ...
  function renderTest(t){$('testTime').textContent=t.timestamp||'Timestamp non disponibile';$('doughTemperature').textContent=value(t.dough_temperature_c,'°C');$('doughStatus').textContent=t.dough_status;$('temperature').textContent=value(t.ambient_temperature_c,'°C');$('humidity').textContent=value(t.humidity_pct,'%');$('distance').textContent=value(t.distance_corrected_mm,'mm',3);$('distanceStatus').textContent=t.distance_status}
