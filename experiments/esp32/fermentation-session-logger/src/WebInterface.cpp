...
<script>
  ...
  async function api(path,options={}){const{timeoutMs=4500,parseJson=true,...fetchOptions}=options;const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);try{const response=await fetch(path,{cache:'no-store',...fetchOptions,signal:controller.signal});if(!response.ok)throw new Error('HTTP '+response.status);if(!parseJson){await response.arrayBuffer();return null}const text=await response.text();return text?JSON.parse(text):null}catch(error){if(error&&error.name==='AbortError')throw new Error('timeout');throw error}finally{clearTimeout(timer)}}
...
void WebInterface::configureRoutes() {
  ...
  server_.on("/api/session/toggle", HTTP_POST, [this]() {
    server_.sendHeader("Cache-Control", "no-store");
    if (!toggleHandler_) {
