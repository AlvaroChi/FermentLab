...
<script>
  ...
  async function refresh(){if(!actionInProgress){try{renderStatus(await api('/api/status'))}catch(e){$('connection').textContent='Offline';$('connection').className='badge';$('notice').textContent='ESP32 non raggiungibile: '+e.message}}setTimeout(refresh,2500)}
