...
Netlist, ERC reports, local KiCad preferences, backup files and experimental
`standard-symbol-test` outputs are generated locally and intentionally not
versioned. The old `.sch`/`.pro` migration inputs are superseded by the native
KiCad 10 project above.
...
the schematic in place to the latest supported format, exported SVG and
netlist, and completed ERC with **0 errors and 0 warnings**. Generated reports
can be recreated locally and are not tracked. This means the
